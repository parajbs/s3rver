...
module.exports = () =>
  async function authentication(ctx, next) {
    ...
    const account = ctx.app.accounts.getByAccessKeyId(request.accessKeyId);
    if (!account) {
      throw new S3Error(
        'InvalidAccessKeyId',
    ...
    if (request.signature.version === 2) {
      ...
      request.signingKey = account.getSecretAccessKey(request.accessKeyId);
      ...
      request.signingKey = v4.getSigningKey(
        account.getSecretAccessKey(request.accessKeyId),
