...
const multipleAccounts = require('./plugins/frontEnd/multipleAccounts');
const webUsers = require('./plugins/frontEnd/webUsers');
const initFrontEnd = require('./plugins/frontEnd/initFrontEnd');
...
class S3rver extends Koa {
  constructor(options) {
    ...
    this.privatebuckets = privatebuckets;
    ...
    this.store = this.context.store = new FilesystemStore(directory);
      ...
      // multipleAccounts			  
    this.multipleAccounts = multipleAccounts(this);
	  
      // WebUsers	
    this.webUsers = webUsers(this);	  

    // Log all requests
    this.use(loggerMiddleware(this, silent, logfile));
    ...
    try {
      ...
      // FrontEnd	
      this.initFrontEnd = initFrontEnd(this);
  ...
  async configureBuckets() {
    return Promise.all(
      this._configureBuckets.map(async (bucket) => {
		let bucketExists = '';
		if (this.privatebuckets === true) {	
			bucketExists = !!(await this.store.getBucketB(bucket.name, this.defaultAccountId));
			} else {
			bucketExists = !!(await this.store.getBucket(bucket.name, this.defaultAccountId));
			}; 
        const replacedConfigs = [];
        await this.store.putBucketB(bucket.name, this.defaultAccountId);
...
S3rver.defaultOptions = {
  ...
  vhostBuckets: true,
  configureBuckets: [],
  defaultAccountId: '123456789000',
  defaultAccountDisplayName: 'S3rver',
