...
//##################	
  getResourcePathB(bucket, key = '', resource) {
    const parts = FilesystemStore.encodeKeyPath(key).split('/');
    const suffix = format(S3RVER_SUFFIX, parts.pop(), resource);
    return path.join(this.rootDirectory, bucket, ...parts, suffix);
  }	
	
  async getcidB(bucket) {
     const cidPath = this.getResourcePathB(bucket, '' ,'bucket.cid'); 	  
    const stat = await fs.stat(cidPath);
    const [cidB] = await Promise.all([	  
      fs
        .readFile(`${cidPath}`)
        .then((cidB) => cidB.toString())
        .catch(async (err) => {
          if (err.code !== 'ENOENT') throw err;
          // create the md5 file if it doesn't already exist
        }),
  ...
  async putcidB(bucket, cid) {
     const cidPath = this.getResourcePathB(bucket, '' ,'bucket.cid'); 
    await fs.writeFile(cidPath, cid);
  }		
//###############	
 ...
 //################################
  async listBucketsB(cid, options) {
	    const {
      delimiter = '',
      startAfter = '',
      prefix = '',
    ...
    for (const bucket of list) {
	  cidB1 = await this.getcidB(bucket);  	
      if (!list) {	
        continue;
      }	
	  if (cid !== cidB1) { 
    ...
    const buckets = await Promise.all(
      keys1.map((key1) => this.getBucket(key1, cid)),
    );
    return buckets.filter(Boolean);
  }

  async getBucketB(bucket, cid) {
	const cidB = this.getcidB(bucket);   
    const bucketPath = this.getBucketPath(bucket);
    try {
      const stat = await fs.stat(bucketPath);
      if (!stat.isDirectory()) return null;
  ...
  async putBucketB(bucket, cid) {
    const bucketPath = this.getBucketPath(bucket);
    await ensureDir(bucketPath);
	await this.putcidB(bucket, cid);  
    return this.getBucket(bucket, cid);
  }
//###############################	
