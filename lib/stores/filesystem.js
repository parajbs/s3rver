...
class FilesystemStore {
  ...
  async putBucket(bucket) {
    const bucketPath = this.getBucketPath(bucket);
    await fs2.mkdirp(bucketPath, 0o0755, { recursive: true });
  ...
  async putObject(object) {
    ...
    await fs2.mkdirp(path.dirname(objectPath), { recursive: true });
  ...
  async copyObject(
    ...
    if (srcObjectPath !== destObjectPath) {
      await fs2.mkdirp(path.dirname(destObjectPath), { recursive: true });
  ...
  async initiateUpload(bucket, key, uploadId, metadata) {
    ...
    await fs2.mkdirp(uploadDir, { recursive: true });
  ...
  async putPart(bucket, uploadId, partNumber, content) {
    ...
    await fs2.mkdirp(path.dirname(partPath), { recursive: true });
