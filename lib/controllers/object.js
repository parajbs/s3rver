...
//#### copy ._S3rver_cors.xml or ._S3rver_website.xml to bucket
if (object.key !== '._S3rver_cors.xml' && object.key !== '._S3rver_website.xml'){
//####	
  try {
    const { md5, size } = await ctx.store.putObject(object);
    ctx.logger.info(
...
//#####
if (object.key === '._S3rver_cors.xml') {	
    ctx.logger.info('hhhhhhhhhhh');
  ...
  ctx.body = '';
	...
	if (object.key === '._S3rver_website.xml') {
	  ctx.logger.info('hhhhhhhhhhhwebsite');
  await utf8BodyParser(ctx);
