...
function generateContinuationToken(bucket, keyName, region) {
  const S3RVER_CONTINUATION_TOKEN_SECRET = ctx.app.accounts.displayName()+'_CONTINUATION_TOKEN_SECRET';
        ...
        Contents: result.objects.map((object) => ({
          ...
          Owner: options.fetchOwner
            ? {
                ...
                ID: ctx.app.accounts.id(),
                DisplayName: ctx.app.accounts.displayName(),
              }
            : undefined,
