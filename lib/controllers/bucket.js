...
exports.bucketExists2 = async function bucketExists2(ctx, next) {
...
exports.putBucket = async function putBucket(ctx) {
  ...
  if (ctx.app.privatebuckets === true) {
    ...
    cid = ctx.state.account.id;	  
    bucket = await ctx.app.store.getBucketB(bucketName, cid);	  
  }	else {
  cid = 'public';
  bucket = await ctx.store.getBucket(bucketName);
  };
  ...
  await ctx.store.putBucketB(bucketName, cid);
